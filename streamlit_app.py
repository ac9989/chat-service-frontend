...
import json
from common import ApiError, SERVICE_NAME, api, conversation_label, auth_headers, SessionExpired, stream_answer

# 상태 종류
# 1. 비로그인
# 2. 로그인 + 대화 없음
# 3. 로그인 + 대화 있음 + 대화 선택 안함 > 제일 위의 대화 선택
# 4. 로그인 + 대화 있음 + 대화 선택 있음 + 메세지 없음 > 예시 질문 출력
# 5. 로그인 + 대화 있음 + 대화 선택 있음 + 메세지 있음 > 대화 화면 출력 + 꼬리질문 출력
# 6. 세션 만료 > 토큰 60분 초과 > 풀린 이유+로그인 안내
...
def render_login() -> None:
    ...
    # api/auth/login,  api/auth/signup  호출
